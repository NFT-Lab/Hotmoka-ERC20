 ...
 * that a supply mechanism has to be added in a derived contract using {@link ERC20#_mint}.
 * For a generic mechanism see {@link ERC20PresetMinterPauser}.
 *
 ...
 *
 * Additionally, an {@link IERC20.Approval} event is emitted on calls to {@link ERC20#transferFrom}.
 * This allows applications to reconstruct the allowance for all accounts just
 * by listening to said events. Other implementations of the EIP may not emit
 * these events, as it isn't required by the specification.
 *
 * Finally, the non-standard {@link ERC20#decreaseAllowance} and {@link ERC20#increaseAllowance}
...
public abstract class ERC20 extends Contract implements IERC20 {
  ...
  public boolean transferFrom(Contract sender, Contract recipient, UnsignedBigInteger amount) {
    ...
    UnsignedBigInteger currentAllowance = _allowances.get(sender).getOrDefault(caller(), new UnsignedBigInteger(0));
    Takamaka.require(currentAllowance.compareTo(amount) >= 0, "ERC20: transfer amount exceeds allowance");
  ...
  public boolean increaseAllowance(Contract spender, UnsignedBigInteger addedValue) {
    _approve(caller(), spender, _allowances.getOrDefault(caller(), new StorageTreeMap<>()).getOrDefault(spender, new UnsignedBigInteger(0)).add(addedValue));
    return true;
  }
  ...
  public boolean decreaseAllowance(Contract spender, UnsignedBigInteger subtractedValue) {
    UnsignedBigInteger currentAllowance = _allowances.getOrDefault(caller(), new StorageTreeMap<>()).getOrDefault(spender, new UnsignedBigInteger(0));
    Takamaka.require(currentAllowance.compareTo(subtractedValue) >= 0, "ERC20: decreased allowance below zero");
  ...
  private void _transfer(Contract sender, Contract recipient, UnsignedBigInteger amount) {
    ...
    UnsignedBigInteger recipientBalance = _balances.getOrDefault(recipient, new UnsignedBigInteger(0));
    Takamaka.require(senderBalance.compareTo(amount) >= 0, "ERC20: transfer amount exceeds balance");
    _balances.put(sender, _balances.getOrDefault(sender, new UnsignedBigInteger(0)).subtract(amount));
  ...
  /** Creates {@code amount} tokens and assigns them to {@code amount}, increasing
   * the total supply.
   *
   * Emits a {@link ERC20.Transfer} event with {@code from} set to the zero address.
   *
  ...
  /**
   * Destroys {@code amount} tokens from {@code account}, reducing the
   * total supply.
   *
   * Emits a {@link ERC20.Transfer} event with {@code to} set to the zero address.
   *
